...
function Page() {
  ...
  useEffect(() => {
    async function getData() {
      setFetching(true);
      try {
        const { paidEntries, paidAdvances } = await getDailyPaidEntries({
          date,
        });
  ...
  const renderTableRow = (item, index, isAdvance = false) => (
    ...
    >
      <td className="border py-2 text-center font-semibold">{index + 1}</td>
      <td className="whitespace-nowrap border py-2 text-center text-lg font-semibold">
        {item.farmername}
      </td>
      <td className="w-auto border py-2 text-center">{item.mobilenumber}</td>
      <td className="w-auto border py-2 text-center">
        {Number(isAdvance ? item.advancecount : item.entrycount).toLocaleString(
          'en-IN'
        )}
      </td>
      <td className="w-auto border py-2 text-center">
      ...
      <td className="w-auto border py-2 text-center font-semibold">
        {Number(
          isAdvance ? item.totaladvance : item.totalpayable
      ...
      <td className="border py-2 text-center">
        {isAdvance ? item.collectedby : item.paidby}
      </td>
    </tr>
  );
  ...
  return (
    <div className="p-2 md:p-14 lg:p-14">
      ...
      <div className="overflow-x-auto rounded-lg bg-white p-8 shadow-lg lg:col-span-3 lg:p-8">
        <h1 className="mb-2 text-center text-2xl font-bold">रोजमेळ</h1>
        ...
        <table className="mt-5 w-full divide-y-2 divide-gray-200 overflow-x-auto bg-white p-8 text-sm">
          <thead className="border-b">
            <tr>
              <th className="border-2 py-2 font-bold text-gray-900">Sr. no</th>
              <th className="border-2 py-2 font-bold text-gray-900">
                Farmer Name
              </th>
              <th className="border-2 py-2 font-bold text-gray-900">Mobile</th>
              <th className="border-2 py-2 font-bold text-gray-900">
                Reciept Count
              </th>
              <th className="border-2 py-2 font-bold text-gray-900">
              ...
              <th className="border-2 py-2 font-bold text-gray-900">Time</th>
              <th className="border-2 py-2 font-bold text-gray-900">Paid By</th>
            </tr>
          </thead>
          ...
          <tbody>
            {fetching ? (
              ...
              <>
                ...
                {paidAdvances.map((item, index) =>
                  renderTableRow(item, paidEntries.length + index, true)
                )}
              </>
            )}
            {(paidEntries.length !== 0 || paidAdvances.length !== 0) && (
              <tr className="border-b">
                <td
                  colSpan="3"
                  className="border py-2 text-center font-bold text-gray-700"
                >
                  Total
                </td>
                <td className="border py-2 text-center font-bold text-gray-700">
                  {(
                    paidEntries.reduce(
                      (acc, item) => acc + Number(item.entrycount),
                      0
                    ) +
                    paidAdvances.reduce(
                      (acc, item) => acc + Number(item.advancecount),
                      0
                    )
                  ).toLocaleString('en-IN')}
                </td>
                ...
                <td className="border py-2 text-center font-bold text-gray-700">
                  {(
                    paidEntries.reduce(
                      (acc, item) => acc + Number(item.totalpayable),
                      0
                    ) +
                    paidAdvances.reduce(
                      (acc, item) => acc + Number(item.totaladvance),
                      0
                    )
                  ).toLocaleString('en-IN')}
                  /-
                </td>
                <td className="border py-2 text-center font-bold text-gray-700"></td>
              </tr>
            )}
          </tbody>
          {(paidEntries.length !== 0 || paidAdvances.length !== 0) && (
            <tfoot>
              <tr>
                ...
                >
                  Cash:{' '}
                  {(paymentTotal.cash + advanceTotal.cash).toLocaleString(
                    'en-IN'
                  )}{' '}
                  /- UPI:{' '}
                  {(paymentTotal.upi + advanceTotal.upi).toLocaleString(
                    'en-IN'
                  )}{' '}
                  /- Mix:{' '}
                  {(paymentTotal.mix + advanceTotal.mix).toLocaleString(
                    'en-IN'
                  )}{' '}
                  /-
                </td>
