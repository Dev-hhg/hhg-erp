  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const commissionSum = res2.reduce(
          (acc, item) => acc + parseInt(item.total_commision, 10),
          0
        );
        ...
        const refundSum = res.reduce(
          (acc, item) => acc + parseInt(item.total_refund_value, 10),
          0
        );
        const avgMonthlyWweight =
          res.reduce((acc, item) => acc + parseInt(item.total_weight, 10), 0) /
          30;
        const avgMonthlyQuantity =
          res.reduce(
            (acc, item) => acc + parseInt(item.total_quantity, 10),
            0
          ) / 30;
  ...
  return (
    <div className="p-14">
      <div className="rounded-md bg-white">
        {/* Filled line chart */}
        <h1 className="mb-2 p-2 text-center text-3xl font-semibold">
          Daily Summary
        </h1>
        ...
        <div className="p-2">
          <div>
            ...
            {!loading && (
              <>
                <div>
                  <div className="mt-2 flex flex-col justify-center sm:space-y-2 md:flex-row md:space-x-8 md:space-y-0 lg:flex-row lg:space-x-4 lg:space-y-0">
                    <Article
                    ...
                    <Article
                      ...
                      val={
                        (data[
                          data.length - 1
                        ]?.total_refund_value).toLocaleString('en-IN')
                    ...
                    <Article
                      valName={
                        commissionDates[commissionDates.length - 1] +
                        ' commission'
                      }
                      val={
                      ...
                      metaVal={
                        commissionData[commissionData.length - 1] -
                        commissionData[commissionData.length - 2]
                          ? 'Rs.' +
                            (
                              commissionData[commissionData.length - 1] -
                              commissionData[commissionData.length - 2]
                    ...
                    <Article
                      ...
                      val={
                        'Rs. ' +
                        (commissionSum + refundSum).toLocaleString('en-IN') +
                        ' /-'
                      }
                      metaVal={''}
                    />
                ...
                <div className="flex flex-col justify-center">
                  <h1 className="5xl text-center font-semibold">
