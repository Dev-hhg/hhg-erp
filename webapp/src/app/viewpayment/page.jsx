...
export default function Page() {
  ...
  useEffect(() => {
    async function getData() {
      const vmData = await getEntriesVmDataByVendor({ vendor, fromDate, toDate });
  ...
  return (
    <div className=" mx-auto p-6">
      <div className="bg-white shadow-md rounded-lg p-6">
        ...
        {alert.show && (
          <div
            className={`p-4 mb-4 rounded-md ${alert.type === "error" ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}
          >
            {alert.message}
          </div>
        )}
