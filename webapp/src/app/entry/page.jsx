...
import { Loader2, Search, Calendar, User, Phone, Package, Scale, Hash } from 'lucide-react'
...
//     `);
	...
	const searchResult = searchedItems.map(({ farmername }, index) => {
		...
		return (
			<li
				...
				}hover:bg-blue-400 hover:text-white p-2`}
				onClick={() => {
					setData((prev) => {
	...
	const handleSearchItem = (e) => {
		...
		const searchResult = fuse.search(search).map((result) => result.item.itemname);
	...
	const searchResultItems = searchedMaal.map((item, index) => {
		...
		return (
			<li
				...
				className={`cursor-pointer ${
					isSel ? "bg-blue-400 text-white" : ""
				}hover:bg-blue-400 hover:text-white p-1 rounded-md max-h-40`}
				onClick={() => {
					setData((prev) => {
              ...
              {recentEntries.map((entry, index) => (
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    <p className="font-semibold text-lg">{entry.farmername}</p>
                    
                  </div>
                  <div className="gridtext-sm text-gray-600 space-y-1">
                    <p><span className="font-medium">UID:</span> {entry.uid}</p>
                    <p><span className="font-medium">Item:</span> {entry.quantity} x {entry.item}</p>
                  </div>
                </div>
              ))}
            </div>
